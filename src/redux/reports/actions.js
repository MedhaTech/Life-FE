...
export const getMentorReport = (queryParams) => async (dispatch) => {
    try {
        ...
        if (result && result.status === 200) {
            const data =
                result.data.data && result.data.data.length > 0
                    ? result.data.data.map((item) => flattenObj(item))
                    : [];
            dispatch(getMentorReportSuccess(data));
            openNotificationWithIcon(
                'success',
                'Mentor report fetched successfully'
            );
        }
    } catch (error) {
...
export const getMentorRegStatusReport =
    (surveyId, role, status) => async (dispatch) => {
        try {
            ...
            if (result && result.status === 200) {
                ...
                openNotificationWithIcon(
                    'success',
                    'Teacher data fetched successfully'
                );
            }
        } catch (error) {
            dispatch(getSurveyReportSuccess([]));
